...
class GetPopularMovies(private val movieRepository: MovieRepository) :
    UseCase<List<Movie>, GetPopularMovies.Params>() {
    override fun run(params: GetPopularMovies.Params?): Either<Failure, List<Movie>> {
        val popularParams = initParams(params)
        val response = movieRepository.getPopular(popularParams.language, popularParams.page, popularParams.region)
