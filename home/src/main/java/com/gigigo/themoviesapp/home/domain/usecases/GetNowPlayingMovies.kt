...
class GetNowPlayingMovies(private val movieRepository: MovieRepository) :
    ...
    override fun run(params: GetNowPlayingMovies.Params?): Either<Failure, List<Movie>> {
        val nowPlayingParams = initParams(params)
        val response = movieRepository.getNowPlaying(nowPlayingParams.language, nowPlayingParams.page, nowPlayingParams.region)
